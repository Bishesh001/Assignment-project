...
if (contactForm) {
    contactForm.addEventListener("submit", (e) => {
        ...
        if (isValid) {
            // Send data in the background using FormSubmit API
            fetch(contactForm.action, {
                ...
                .then(response => {
                    if (response.ok) {
                        ...
                        alert("Transmission failed. S.H.I.E.L.D encryption blocked the message.");
                    }
                })
                .catch(error => alert("Transmission failed. Try again Agent."));
        }
    });
}
